...
export function bindOwner() {
  ...
  const systemTone = (value) =>
    value === "HEALTHY" || value === "GOOD" || value === "CONNECTED" || value === "READY" || value === "ONLINE" || value === "RUNNING" || value === "SHADOW_RUNNING"
      ? "green"
  ...
  function renderPage(data) {
    ...
    const local = data.local_worker ?? { online:false,state:"UNKNOWN",host_type:null,runtime_mode:null,build_sha:null,
      ...
      alpaca_health:"UNKNOWN",optionomics_health:"UNKNOWN",database_health:"UNKNOWN",execution_gate:"LOCKED" };
    ...
    if (page === "/ops/theta")
      return `${title}<div class="ops-control-grid">${opsGroup("Local shadow worker", "The Windows laptop supervisor triggers read-only canonical Production cycles. It has no broker mutation surface.", [["Worker", local.online ? "ONLINE" : local.state, systemTone(local.online ? "ONLINE" : local.state)], ["Mode", local.runtime_mode ?? "UNKNOWN", "blue"], ["Market", local.market_session], ["Last heartbeat", display(local.last_heartbeat,date)], ["Build", local.build_sha ?? "UNKNOWN"], ["Execution", local.execution_gate, "amber"]])}${opsGroup("Current action", "No action is inferred without a running scheduler and persisted decision.", [["Last decision", data.runtime_detail.last_decision ?? "UNKNOWN"], ["Last scan", display(data.runtime_detail.last_scan,date)], ["AEGIS", system("aegis"), systemTone(system("aegis"))], ["Execution authority", data.trading, "amber"]])}${opsGroup("Decision pipeline", "Deterministic contracts exist. Runtime inputs and orchestration remain gated.", [["Runtime stage", data.runtime_detail.stage, "amber"], ["Last snapshot", data.runtime_detail.last_market_snapshot ?? "UNKNOWN"], ["Last reconciliation", display(local.last_reconciliation,date)], ["Candidates evaluated", data.runtime_detail.candidates_evaluated ?? "UNKNOWN"], ["Policy version", data.runtime_detail.policy_version ?? "NOT ACTIVE"]])}${opsGroup("Economic truth", "Whole-chain accounting includes option and assigned-stock economics.", [["Ledger runtime", system("ledger"), systemTone(system("ledger"))], ["Whole-chain P&L", "UNKNOWN"], ["Drawdown", "UNKNOWN"], ["After-cost EV", "UNKNOWN"], ["Open inventory MTM", "UNKNOWN"]])}</div>`;
    if (page === "/ops/trading")
    ...
    return `${title}<section class="ops-section"><div class="section-heading"><div><h2>THETA runtime</h2><p>Provider checks run independently. Order submission remains locked.</p></div>${badge(data.runtime_detail.stage, local.online ? "green" : "amber")}</div><dl class="capability-grid"><div><dt>Website</dt><dd>${badge(data.website, "blue")}</dd></div><div><dt>Master mode</dt><dd>${badge(data.bot_mode, "blue")}</dd></div><div><dt>THETA Shadow</dt><dd>${badge(local.online ? "ONLINE" : local.state, systemTone(local.online ? "ONLINE" : local.state))}</dd></div><div><dt>Execution</dt><dd>${badge(data.execution_control.master_paper_execution, "amber")}</dd></div></dl></section>${readinessPanel()}<div class="ops-control-grid">${opsGroup("Local laptop", "Pinned canonical runtime with a durable lease and restart-safe scheduling.", [["Heartbeat", display(local.last_heartbeat,date)], ["Build", local.build_sha ?? "UNKNOWN"], ["Market session", local.market_session], ["Alpaca", local.alpaca_health, systemTone(local.alpaca_health)], ["Database", local.database_health, systemTone(local.database_health)], ["Execution gate", local.execution_gate, "amber"]])}${opsGroup("Master THETA", "Independent platform account and research providers.", [["Alpaca Paper", data.master_connection.connection_state, systemTone(data.master_connection.connection_state)], ["Optionomics", data.provider_runtime], ["THETA Shadow", local.runtime_mode ?? "UNKNOWN", "blue"], ["Database", data.database.state, systemTone(data.database.state)], ["Market data", system("market_data")], ["Execution", data.execution_control.master_paper_execution, "amber"]])}${opsGroup("Customer Copy", "Separate follower connection and persistence readiness.", [["Customer IAM", data.copy_platform.customer_iam, systemTone(data.copy_platform.customer_iam)], ["Private Paper Beta", data.copy_platform.private_paper_beta.state, "amber"], ["Alpaca Connect", data.copy_platform.oauth, systemTone(data.copy_platform.oauth)], ["Token vault", data.copy_platform.token_vault, systemTone(data.copy_platform.token_vault)], ["Follower adapter", data.copy_platform.follower_broker_adapter, "green"], ["Copy engine", data.copy_platform.copy_execution, "amber"], ["Follower execution", data.execution_control.follower_paper_execution, "amber"]])}${opsGroup("Current action", "No decision is displayed without persisted runtime evidence.", [["Last decision", data.runtime_detail.last_decision ?? "UNKNOWN"], ["Last scan", display(data.runtime_detail.last_scan,date)]])}${opsGroup("Attention", "Select a verification above or resolve these release gates.", data.gates.map((gate) => [gate, "OPEN", "amber"]))}</div>`;
  }
