...
export function bindOwner() {
  ...
  const systemTone = (value) =>
    value === "HEALTHY" || value === "GOOD" || value === "CONNECTED" || value === "READY" || value === "ONLINE" || value === "RUNNING" || value === "MASTER_PAPER_ACTIVE" || value === "MASTER_PAPER_MARKET_CLOSED"
      ? "green"
  ...
  const signOut =
    ...
    `<section class="operator-readiness"><div><h2>Readiness verification</h2><p>Run independent read-only checks from the deployed server.</p></div><div class="heading-actions"><button id="verify-master" class="button secondary">Verify Alpaca Paper</button><button id="verify-optionomics" class="button secondary">Test Optionomics</button><button id="qualify-optionomics" class="button secondary">Qualify Optionomics</button><button id="verify-database" class="button secondary">Verify database</button></div><div id="master-result" aria-live="polite"></div><div id="optionomics-result" aria-live="polite"></div><div id="database-result" aria-live="polite"></div></section>`;
  const safetyControls = (data) => `<section class="panel operator-readiness"><div><h2>Runtime safety controls</h2><p>These controls can reduce or stop risk. They cannot enable execution, promote a policy, or clear the external quote gate.</p></div><div class="heading-actions"><button id="pause-new-entries" class="button secondary">Pause new entries</button><button id="resume-new-entries" class="button secondary">Resume new entries</button><button id="emergency-lock" class="button secondary">Emergency execution lock</button></div><p id="operator-control-result" class="caption" aria-live="polite">New entries: ${data.execution_control.pause_new_orders ? "paused" : "allowed by operator state"}. Emergency lock: ${data.execution_control.emergency_execution_lock ? "on" : "off"}.</p></section>`;

  function renderPage(data) {
    const system = (name) => data.systems[name] ?? "UNKNOWN";
    const display = (item, formatter = String) => item == null ? "UNKNOWN" : formatter(item);
    const local = data.local_worker ?? { online:false,state:"UNKNOWN",host_type:null,runtime_mode:null,build_sha:null,
      last_heartbeat:null,last_cycle_completed:null,last_reconciliation:null,last_candidate_scan:null,market_session:"UNKNOWN",
      alpaca_health:"UNKNOWN",optionomics_health:"UNKNOWN",database_health:"UNKNOWN",execution_gate:"EXTERNAL_QUOTE_BLOCKER" };
    const evidence = data.runtime_evidence ?? { broker_orders:null,broker_fills:null,open_chains:null,
      option_realized_pnl:null,stock_realized_pnl:null,whole_chain_pnl:null,strategy_branch:null };
    const behavior = data.runtime_detail.behavior_diagnostic ?? {};
    const waitFunnel = opsGroup("Decision funnel", "Observational evidence distinguishes selective WAIT from possible decision paralysis. It cannot authorize a trade.", [
      ["Classification", behavior.wait_classification ?? "UNKNOWN", behavior.wait_classification === "HEALTHY_WAIT" ? "green" : "amber"],
      ["Final action", behavior.final_action ?? "UNKNOWN"], ["Session", behavior.session ?? "UNKNOWN"],
      ["Universe", behavior.universe_size ?? "UNKNOWN"], ["Strategies applicable", behavior.strategies_applicable ?? "UNKNOWN"],
      ["Candidates generated", behavior.candidate_count ?? "UNKNOWN"], ["Hard rejections", behavior.hard_rejected_count ?? "UNKNOWN"],
      ["Soft economic rejections", behavior.soft_economic_rejection_count ?? "UNKNOWN"],
      ["Unknown-data rejections", behavior.data_unknown_rejection_count ?? "UNKNOWN"],
      ["Quote rejections", behavior.quote_rejection_count ?? "UNKNOWN"], ["Liquidity rejections", behavior.liquidity_rejection_count ?? "UNKNOWN"],
      ["AEGIS rejections", behavior.aegis_veto_count ?? "UNKNOWN"], ["Near misses", behavior.near_miss_count ?? "UNKNOWN"],
      ["Findings", Array.isArray(behavior.anti_paralysis_findings) && behavior.anti_paralysis_findings.length ? behavior.anti_paralysis_findings.join(", ") : "NONE"]
    ]);
    ...
    if (page === "/ops/theta")
      return `${title}<div class="ops-control-grid">${opsGroup("Master Paper worker", "The Windows laptop supervisor runs canonical reconciliation, management, scanning, and evidence cycles without depending on followers.", [["Worker", local.online ? "ONLINE" : local.state, systemTone(local.online ? "ONLINE" : local.state)], ["Mode", local.runtime_mode ?? "UNKNOWN", "blue"], ["Market", local.market_session], ["Last heartbeat", display(local.last_heartbeat,date)], ["Build", local.build_sha ?? "UNKNOWN"], ["Execution", local.execution_gate, "amber"]])}${opsGroup("Execution tier", "Paper evidence can collect bounded execution evidence without claiming empirical promotion or live eligibility.", [["Tier", data.execution_control.execution_tier, "blue"], ["Empirical promotion", data.execution_control.empirical_promotion_tier, "amber"], ["Expected after-cost EV", data.execution_control.expected_after_cost_ev_state], ["Evidence risk cap", data.execution_control.paper_evidence_risk_cap], ["Quote gate", data.execution_control.execution_quote_gate, "amber"], ["Live eligible", data.execution_control.live_eligible ? "YES" : "NO", "amber"]])}${opsGroup("Current action", "Only persisted master runtime evidence is displayed.", [["Last decision", data.runtime_detail.last_decision ?? "UNKNOWN"], ["Strategy", evidence.strategy_branch ?? "UNKNOWN"], ["Last scan", display(data.runtime_detail.last_scan,date)], ["AEGIS", system("aegis"), systemTone(system("aegis"))], ["Execution authority", data.trading, "amber"]])}${opsGroup("Decision pipeline", "Broker reconciliation and management run before new opportunity discovery.", [["Runtime stage", data.runtime_detail.stage, "amber"], ["Last snapshot", data.runtime_detail.last_market_snapshot ?? "UNKNOWN"], ["Last reconciliation", display(local.last_reconciliation,date)], ["Candidates evaluated", data.runtime_detail.candidates_evaluated ?? "UNKNOWN"], ["Policy version", data.runtime_detail.policy_version ?? "NOT ACTIVE"]])}${waitFunnel}${opsGroup("Economic truth", "Whole-chain P&L stays unknown when open inventory lacks a current mark.", [["Ledger runtime", system("ledger"), systemTone(system("ledger"))], ["Whole-chain P&L", evidence.whole_chain_pnl == null ? "UNKNOWN" : money(evidence.whole_chain_pnl)], ["Option realized", evidence.option_realized_pnl == null ? "UNKNOWN" : money(evidence.option_realized_pnl)], ["Stock realized", evidence.stock_realized_pnl == null ? "UNKNOWN" : money(evidence.stock_realized_pnl)], ["Open chains", display(evidence.open_chains)]])}</div>`;
    if (page === "/ops/trading")
      return `${title}${readinessPanel()}${safetyControls(data)}<div class="ops-control-grid">${opsGroup("Positions", "Broker-reconciled master PAPER exposure only.", [["Open positions", data.runtime_detail.open_positions ?? "UNKNOWN"], ["Open THETA chains", display(evidence.open_chains)], ["Assignment inventory", "UNKNOWN"], ["Recovery positions", "UNKNOWN"], ["Corporate actions", "UNKNOWN"]])}${opsGroup("Orders and fills", "Paper evidence submission requires every action-level safety gate and a qualified Alpaca execution quote.", [["Execution tier", data.execution_control.execution_tier, "blue"], ["Paper evidence eligible", data.execution_control.paper_evidence_eligible, "amber"], ["Master paper execution", data.execution_control.master_paper_execution, "amber"], ["Pause new orders", data.execution_control.pause_new_orders ? "ON" : "OFF", data.execution_control.pause_new_orders ? "amber" : "green"], ["Emergency lock", data.execution_control.emergency_execution_lock ? "ON" : "OFF", data.execution_control.emergency_execution_lock ? "amber" : "green"], ["Pending orders", data.runtime_detail.pending_orders ?? "UNKNOWN"], ["Recorded broker orders", display(evidence.broker_orders)], ["Recorded fills", display(evidence.broker_fills)], ["Unknown submissions", data.runtime_detail.unknown_submissions ?? "UNKNOWN"], ["Reconciliation", system("reconciliation"), systemTone(system("reconciliation"))]])}</div>`;
    if (page === "/ops/copy")
      return `${title}<div class="ops-control-grid">${opsGroup("Copy engine", "Master-fill-first follower plans, account-specific sizing, AEGIS evidence and reconciliation are durable. Broker submission remains locked.", [["Contract", "READY", "green"], ["Database", data.database.state, systemTone(data.database.state)], ["Execution", system("copy_engine"), systemTone(system("copy_engine"))], ["Follower execution", data.execution_control.follower_paper_execution, "amber"], ["Action plans", data.database.follower_action_plans ?? "UNKNOWN"], ["Reconciliation events", data.database.follower_reconciliation_events ?? "UNKNOWN"], ["Divergence events", data.database.follower_divergence_events ?? "UNKNOWN"], ["Runtime checkpoints", data.database.follower_runtime_checkpoints ?? "UNKNOWN"], ["Quantity zero", "VALID", "green"]])}${opsGroup("Followers", "Connections remain tenant-scoped and independently verified.", [["Connected followers", data.database.active_followers ?? "UNKNOWN"], ["Private beta followers", data.copy_platform.private_paper_beta.follower_count], ["Alpaca Connect", data.copy_platform.oauth], ["Token vault", data.copy_platform.token_vault], ["Follower adapter", data.copy_platform.follower_broker_adapter]])}${opsGroup("Lifecycle divergence", "Follower outcomes are never assumed to match the master. Missed entries, partial fills, assignment, recovery, covered calls and call-away are recorded independently.", [["Master-fill-first", "ENFORCED", "green"], ["Follower quote", "REQUIRED", "green"], ["Follower AEGIS", "REQUIRED", "green"], ["Pause behavior", "MANAGE EXISTING", "green"], ["Submission", "LOCKED", "amber"]])}${opsGroup("Private Paper Beta", data.copy_platform.private_paper_beta.reason, [["State", data.copy_platform.private_paper_beta.state, "amber"], ["Policy", data.copy_platform.private_paper_beta.policy_status, "amber"], ["Raw-key endpoint", "UNAVAILABLE"], ["Public raw-key form", "PROHIBITED"]])}</div>`;
    if (page === "/ops/system") {
      const provider=data.provider_qualification??{secret_state:"NOT_CONFIGURED",qualified_capabilities:null,blocked_capabilities:null,real_payload_count:null,stale_capability_count:null,last_check:null};
      const alerts=data.active_alerts??[];
      const alertEntries=[["Active count",alerts.length,alerts.some((item)=>item.severity==="CRITICAL")?"amber":""],...alerts.slice(0,5).map((item)=>[item.type,`${item.severity} · ${item.count}`,item.severity==="CRITICAL"?"amber":""] )];
      return `${title}<div class="ops-health">${Object.entries(data.systems).map(([name, state]) => `<article><span>${esc(name.replaceAll("_", " "))}</span>${badge(state, systemTone(state))}</article>`).join("")}</div>${readinessPanel()}<div class="ops-control-grid">${opsGroup("Optionomics qualification", "Capability families qualify independently. Authentication never authorizes execution.", [["Credential state",provider.secret_state,provider.secret_state==="AUTH_VALID"?"green":"amber"],["Qualified families",display(provider.qualified_capabilities)],["Blocked families",display(provider.blocked_capabilities)],["Real payloads",display(provider.real_payload_count)],["Stale families",display(provider.stale_capability_count)],["Last check",display(provider.last_check,date)]])}${opsGroup("Active alerts", "Alerts are deduplicated state evidence, not an execution command.", alertEntries)}</div><section class="panel release-gates"><h2>Release gates</h2><ul>${data.gates.map((gate) => `<li>${esc(gate)}</li>`).join("")}</ul></section><p class="caption">${esc(data.security)}</p>`;
    }
    return `${title}<section class="ops-section"><div class="section-heading"><div><h2>THETA master Paper runtime</h2><p>The worker remains active across market sessions. Order submission waits for qualified execution pricing.</p></div>${badge(data.runtime_detail.stage, local.online ? "green" : "amber")}</div><dl class="capability-grid"><div><dt>Website</dt><dd>${badge(data.website, "blue")}</dd></div><div><dt>Master mode</dt><dd>${badge(data.bot_mode, "blue")}</dd></div><div><dt>Master worker</dt><dd>${badge(local.online ? "ONLINE" : local.state, systemTone(local.online ? "ONLINE" : local.state))}</dd></div><div><dt>Execution</dt><dd>${badge(data.execution_control.master_paper_execution, "amber")}</dd></div></dl></section>${readinessPanel()}<div class="ops-control-grid">${opsGroup("Local laptop", "Pinned canonical runtime with a durable lease, wake support, and restart-safe scheduling.", [["Heartbeat", display(local.last_heartbeat,date)], ["Build", local.build_sha ?? "UNKNOWN"], ["Market session", local.market_session], ["Alpaca", local.alpaca_health, systemTone(local.alpaca_health)], ["Database", local.database_health, systemTone(local.database_health)], ["Execution gate", local.execution_gate, "amber"]])}${opsGroup("Master THETA", "Independent master account. Follower count cannot pause or alter it.", [["Alpaca Paper", data.master_connection.connection_state, systemTone(data.master_connection.connection_state)], ["Optionomics", data.provider_runtime], ["Runtime", local.runtime_mode ?? "UNKNOWN", "blue"], ["Database", data.database.state, systemTone(data.database.state)], ["Market data", system("market_data")], ["Execution", data.execution_control.master_paper_execution, "amber"]])}${opsGroup("Customer Copy", "Followers remain downstream, isolated, and master-fill-first.", [["Customer IAM", data.copy_platform.customer_iam, systemTone(data.copy_platform.customer_iam)], ["Private Paper Beta", data.copy_platform.private_paper_beta.state, "amber"], ["Alpaca Connect", data.copy_platform.oauth, systemTone(data.copy_platform.oauth)], ["Token vault", data.copy_platform.token_vault, systemTone(data.copy_platform.token_vault)], ["Follower adapter", data.copy_platform.follower_broker_adapter, "green"], ["Copy engine", data.copy_platform.copy_execution, "amber"], ["Follower execution", data.execution_control.follower_paper_execution, "amber"]])}${opsGroup("Current action", "Only persisted runtime evidence is shown.", [["Last decision", data.runtime_detail.last_decision ?? "UNKNOWN"], ["Last scan", display(data.runtime_detail.last_scan,date)], ["Recorded orders", display(evidence.broker_orders)], ["Recorded fills", display(evidence.broker_fills)]])}${opsGroup("Attention", "Select a verification above or resolve these release gates.", data.gates.map((gate) => [gate, "OPEN", "amber"]))}</div>`;
  }

  function bindRuntimeActions(data) {
    const command = async (commandName, message) => {
      if (!globalThis.confirm(message)) return;
      const target=document.querySelector("#operator-control-result");
      try {
        const response=await fetch("/api/v1/operator/controls",{method:"POST",headers:{"Content-Type":"application/json","Idempotency-Key":crypto.randomUUID()},body:JSON.stringify({command:commandName,confirmed:true,observed_state_version:data.execution_control.operator_control.stateVersion,reason:null})});
        const payload=await response.json();
        if(!response.ok)throw new Error(payload.error?.code??"Control update failed");
        data.execution_control.operator_control=payload.data;
        target.textContent=`Saved at state version ${payload.data.stateVersion}. New entries: ${payload.data.newEntriesPaused?"paused":"operator-resumed"}. Emergency lock: ${payload.data.emergencyExecutionLock?"on":"off"}. Execution authority remains false.`;
      } catch(error) { target.textContent=error.message; }
    };
    document.querySelector("#pause-new-entries")?.addEventListener("click",()=>command("PAUSE_NEW_ENTRIES","Pause all new THETA entries? Existing positions will remain managed and reconciled."));
    document.querySelector("#resume-new-entries")?.addEventListener("click",()=>command("RESUME_NEW_ENTRIES","Resume the operator-level new-entry state? Environment, quote, risk, and policy gates will still apply."));
    document.querySelector("#emergency-lock")?.addEventListener("click",()=>command("EMERGENCY_EXECUTION_LOCK","Apply the emergency execution lock? This cannot be cleared by the ordinary resume control."));
    ...
    document.querySelector("#qualify-optionomics")?.addEventListener("click", async (event) => {
      const button=event.currentTarget;button.disabled=true;button.textContent="Qualifying…";const target=document.querySelector("#optionomics-result");
      try{const response=await fetch("/api/v1/operator/optionomics-qualification",{method:"POST",signal:AbortSignal.timeout(30000)});
        const payload=await response.json();if(!response.ok&&response.status!==207)throw new Error(payload.error?.code??"Qualification unavailable");
        const result=payload.data;target.innerHTML=`<p class="notice ${result.secret_state==="AUTH_VALID"?"green":"amber"}"><strong>OPTIONOMICS QUALIFICATION · ${esc(result.secret_state)}</strong><br>${esc(result.families.length)} families checked · ${esc(result.real_payload_count)} real payloads · execution remains locked</p>`;
      }catch(error){target.innerHTML=`<p class="notice amber">${esc(error.message)} No order was submitted.</p>`;}finally{button.disabled=false;button.textContent="Qualify Optionomics";}
    });
