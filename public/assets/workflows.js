...
export function bindOwner() {
  ...
  function bindRuntimeActions() {
    const command = async (commandName, message) => {
      ...
      try {
        const response=await fetch("/api/v1/operator/controls",{method:"POST",headers:{"Content-Type":"application/json","Idempotency-Key":crypto.randomUUID()},body:JSON.stringify({command:commandName,confirmed:true})});
        const payload=await response.json();
        if(!response.ok)throw new Error(payload.error?.code??"Control update failed");
        target.textContent=`Saved. New entries: ${payload.data.newEntriesPaused?"paused":"operator-resumed"}. Emergency lock: ${payload.data.emergencyExecutionLock?"on":"off"}. Execution authority remains false.`;
