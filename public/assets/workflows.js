...
export function bindOwner() {
  ...
  function renderPage(data) {
    ...
    if (page === "/ops/trading")
      return `${title}${readinessPanel()}<div class="ops-control-grid">${opsGroup("Positions", "Broker-reconciled master PAPER exposure only.", [["Open positions", data.runtime_detail.open_positions ?? "UNKNOWN"], ["Open THETA chains", display(evidence.open_chains)], ["Assignment inventory", "UNKNOWN"], ["Recovery positions", "UNKNOWN"], ["Corporate actions", "UNKNOWN"]])}${opsGroup("Orders and fills", "Paper evidence submission requires every action-level safety gate and a qualified Alpaca execution quote.", [["Execution tier", data.execution_control.execution_tier, "blue"], ["Paper evidence eligible", data.execution_control.paper_evidence_eligible, "amber"], ["Master paper execution", data.execution_control.master_paper_execution, "amber"], ["Pause new orders", data.execution_control.pause_new_orders ? "ON" : "OFF", data.execution_control.pause_new_orders ? "amber" : "green"], ["Pending orders", data.runtime_detail.pending_orders ?? "UNKNOWN"], ["Recorded broker orders", display(evidence.broker_orders)], ["Recorded fills", display(evidence.broker_fills)], ["Unknown submissions", data.runtime_detail.unknown_submissions ?? "UNKNOWN"], ["Reconciliation", system("reconciliation"), systemTone(system("reconciliation"))]])}</div>`;
    if (page === "/ops/copy")
