...
function detailHeader(section) {
  return `<div class="theta-context"><div class="bot-identity"><div class="bot-monogram theta" aria-hidden="true">Θ</div><div><p class="eyebrow">PREMIUM INCOME</p><div class="identity-line"><h1>THETA</h1>${badge("PAPER TESTING", "blue")}</div><p>Assignment-aware options income</p></div></div><div class="heading-actions">${link("/bots/theta/copy", "Copy THETA", "button primary")}${link("/bots/theta/how-it-works", "About THETA", "button secondary")}</div></div><div class="theta-context-copy"><p>THETA selects and manages eligible cash-secured puts, assignment, stock recovery, and covered calls as one complete position.</p><span>THETA is building its paper track record.</span></div><nav class="tabs theta-tabs" aria-label="THETA sections">${botTabs.map(([key, text]) => `<a href="/bots/theta${key ? "/" + key : ""}${q}" ${section === key ? 'aria-current="page"' : ""}>${text}</a>`).join("")}</nav>`;
}
function dataState() {
  const messages = [];
