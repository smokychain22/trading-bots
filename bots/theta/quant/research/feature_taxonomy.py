"""Fine-grained feature-role taxonomy for THETA (R6E/R6F).
...
ROLE classification (R6E), plus a second, orthogonal FAMILY dimension (R6F,
`THETA_STRATEGY_ENGINE_AND_IMPLEMENTATION_SPEC_v1.0` section 6): every
feature has both "what job does it serve" (family) and "how strictly does
it act" (role). A feature has an explicit destination on BOTH axes rather
than defaulting into "entry gating" simply because it exists. This is the
concrete mechanism against "25 independent AND conditions -> almost zero
candidates": a feature classified MANAGEMENT_ONLY_FEATURE or
...
class FeatureFamily(str, Enum):
    """R6F, spec section 6's own family list -- "what job does this feature
    serve," orthogonal to FeatureRole's "how strictly does it act.\""""
...
# The feature destination map (R6E item 5, R6F item 17): every feature named
# across this phase's research corpus (friend-bot audit, flow comparison,
# GEX matrix, IV comparison, GitHub formula catalog, hypotheses.json, spec
# section 6) gets exactly one row on BOTH axes. A feature not in this dict
# has no assigned destination YET -- that is a flagged gap, never silently
# defaulted to entry gating.
FEATURE_DESTINATION_MAP: Dict[str, FeatureDestination] = {
    ...
    "critical_data_validity": FeatureDestination(
        "critical_data_validity", FeatureRole.HARD_GATE, FeatureFamily.EXECUTION,
    ...
    "aegis_hard_veto": FeatureDestination(
        "aegis_hard_veto", FeatureRole.HARD_GATE, FeatureFamily.PORTFOLIO_RISK,
        "AEGIS aggregation already a hard, blanket exclusion.",
        "AVAILABLE",
    ),
    "spread_executability": FeatureDestination(
        "spread_executability", FeatureRole.HARD_GATE, FeatureFamily.EXECUTION,
    ...
    "lifecycle_state": FeatureDestination(
        "lifecycle_state", FeatureRole.STRUCTURE_ROUTER_INPUT, FeatureFamily.MANAGEMENT,
    ...
    "stock_shares_held": FeatureDestination(
        "stock_shares_held", FeatureRole.STRUCTURE_ROUTER_INPUT, FeatureFamily.OWNERSHIP,
    ...
    "event_near": FeatureDestination(
        "event_near", FeatureRole.STRUCTURE_ROUTER_INPUT, FeatureFamily.EVENT,
    ...
    "ownership_acceptability": FeatureDestination(
        "ownership_acceptability", FeatureRole.SOFT_FEATURE, FeatureFamily.OWNERSHIP,
    ...
    "iv_rank": FeatureDestination(
        "iv_rank", FeatureRole.SOFT_FEATURE, FeatureFamily.VOLATILITY,
    ...
    "rsi_trend_momentum": FeatureDestination(
        "rsi_trend_momentum", FeatureRole.SOFT_FEATURE, FeatureFamily.UNDERLYING,
        "R6E item 18 / R6F item 18: 'quality-first CSP engine' proposed RSI 40-60 as a hard rule -- reclassified here as a soft underlying-state feature, a hypothesis to ablate, never a mandatory band.",
        "NOT_TESTED",
    ),
    # Soft bonuses / penalties (a feature that structurally only ever pushes one direction)
    "sweep_classification": FeatureDestination(
        "sweep_classification", FeatureRole.SOFT_BONUS, FeatureFamily.FLOW_CONTEXT,
    ...
    "severe_drawdown_probability": FeatureDestination(
        "severe_drawdown_probability", FeatureRole.SOFT_PENALTY, FeatureFamily.OWNERSHIP,
    ...
    "recovery_median": FeatureDestination(
        "recovery_median", FeatureRole.MANAGEMENT_ONLY_FEATURE, FeatureFamily.MANAGEMENT,
    ...
    "call_away_regret": FeatureDestination(
        "call_away_regret", FeatureRole.MANAGEMENT_ONLY_FEATURE, FeatureFamily.MANAGEMENT,
    ...
    "concentration_cluster_exposure": FeatureDestination(
        "concentration_cluster_exposure", FeatureRole.RISK_ONLY_FEATURE, FeatureFamily.PORTFOLIO_RISK,
    ...
    "assignment_capacity": FeatureDestination(
        "assignment_capacity", FeatureRole.RISK_ONLY_FEATURE, FeatureFamily.PORTFOLIO_RISK,
    ...
    "quote_spread": FeatureDestination(
        "quote_spread", FeatureRole.EXECUTION_ONLY_FEATURE, FeatureFamily.EXECUTION,
    ...
    "quote_freshness": FeatureDestination(
        "quote_freshness", FeatureRole.EXECUTION_ONLY_FEATURE, FeatureFamily.EXECUTION,
    ...
    "open_interest_volume": FeatureDestination(
        "open_interest_volume", FeatureRole.EXECUTION_ONLY_FEATURE, FeatureFamily.CONTRACT,
    ...
    "gex_zero_gamma": FeatureDestination(
        "gex_zero_gamma", FeatureRole.SOFT_FEATURE, FeatureFamily.FLOW_CONTEXT,
    ...
    # R6F additions -- spec section 6/8/9 features not yet classified
    "expected_move": FeatureDestination(
        "expected_move", FeatureRole.SOFT_FEATURE, FeatureFamily.VOLATILITY,
        "S*IV*sqrt(T) diagnostic (spec section 9/Appendix A) -- a soft context/diagnostic, never a guaranteed range or literal boundary.",
        "NOT_IMPLEMENTED",
    ),
    "delta_greeks": FeatureDestination(
        "delta_greeks", FeatureRole.SOFT_FEATURE, FeatureFamily.CONTRACT,
        "Delta/gamma/theta/vega -- contract risk/management context, never literal win probability (spec section 6's own explicit guardrail).",
        "AVAILABLE",
    ),
    "trend_regime": FeatureDestination(
        "trend_regime", FeatureRole.SOFT_FEATURE, FeatureFamily.REGIME,
        "Bull/bear/sideways persistence/confidence -- candidate ranking and branch applicability context (spec section 8), never a universal hard gate.",
        "NOT_TESTED",
    ),
    "earnings_distance": FeatureDestination(
        "earnings_distance", FeatureRole.HARD_GATE, FeatureFamily.EVENT,
        "Conventional THETA-Q default excludes earnings when fresh event data is required (spec section 13) -- hard by default for THETA_CONVENTIONAL; event-tolerant variants would need separate OOS evidence to soften it.",
        "AVAILABLE",
    ),
    "ex_div_distance": FeatureDestination(
        "ex_div_distance", FeatureRole.SOFT_PENALTY, FeatureFamily.EVENT,
        "Early-assignment/call-away risk context for THETA-A/THETA-C (spec section 24) -- a penalty, never a bonus.",
        "AVAILABLE",
    ),
    "capital_days": FeatureDestination(
        "capital_days", FeatureRole.MANAGEMENT_ONLY_FEATURE, FeatureFamily.MANAGEMENT,
        "Opportunity-cost accounting metric (spec section 25/40) -- feeds ReturnPerCapitalDay and management utility only, never an entry gate.",
        "AVAILABLE",
    ),
    "fill_probability": FeatureDestination(
        "fill_probability", FeatureRole.EXECUTION_ONLY_FEATURE, FeatureFamily.EXECUTION,
        "UNKNOWN until calibrated from real TCA (spec section 6/26) -- execution utility input only, never a fabricated confidence.",
        "NOT_IMPLEMENTED",
    ),
    "expert_prior": FeatureDestination(
        "expert_prior", FeatureRole.SOFT_FEATURE, FeatureFamily.UNDERLYING,
        "Reliability-weighted trader/GitHub prior (spec section 6/28) -- soft only, cannot override AEGIS or a negative independent EV.",
        "AVAILABLE",
    ),
}
...
def count_by_family() -> Dict[FeatureFamily, int]:
    counts: Dict[FeatureFamily, int] = {family: 0 for family in FeatureFamily}
    for destination in FEATURE_DESTINATION_MAP.values():
        counts[destination.family] += 1
    return counts
