...
_ITEMS: Tuple[PhaseItem, ...] = (
    ...
    PhaseItem("R7_EVENT_STATE_CHANGE", PhaseState.COMPLETE, ("event_state_research.py",),
              note="R7 P2C pass-2 directive: formalizes EVENT_STATE_CHANGE as a seven-state model (NO_KNOWN_EVENT/EVENT_APPROACHING/EVENT_INSIDE_CONTRACT_LIFE/EVENT_IMMINENT/EVENT_OCCURRED/POST_EVENT_NORMALIZATION/EVENT_DATA_UNKNOWN) derived only from caller-supplied timestamps -- never inferred from IV/skew/flow behavior. NO_KNOWN_EVENT requires explicit confirmed_no_event=True; a merely-missing event timestamp is EVENT_DATA_UNKNOWN, keeping 'confirmed absent' and 'unknown' structurally distinct. Also derives the event temporal features (DAYS_TO_EVENT, EXPIRY_CROSSES_EVENT, IV_CRUSH_AFTER_EVENT, etc.) as pure research features carrying no trade authority. See docs/research/THETA_EVENT_STATE_RESEARCH.md. 16 tests."),
    PhaseItem("R7_EXPOSURE_TEMPORAL_CONVENTION", PhaseState.COMPLETE, ("exposure_temporal_convention_research.py",),
              note="R7 P2C pass-2 directive: DEX/Vanna/Charm deep research (definitions/units/sign-convention distinctness, never a directional trade rule) plus a temporal-change interface mirroring optionomics_flow_temporal_research.py's discipline, extended to refuse diffing two observations under incompatible sign conventions or mismatched provenance (PROVIDER_FACT vs THETA_DERIVED) -- prevents silently fabricating a 'change' that is actually just a methodology switch. 9 tests."),
    PhaseItem("R7_EXECUTION_TCA", PhaseState.COMPLETE, ("execution_tca_research.py",),
              note="R7 P2C pass-2 directive: six-model fill-sensitivity band (MID/BID_SIDE/ASK_SIDE/SPREAD_FRACTION/LIQUIDITY_ADJUSTED/PER_LEG_SLIPPAGE, every model returned side by side, never one picked as truth), multi-leg fill modeling (SIMULTANEOUS_PACKAGE vs INDEPENDENT_LEGGING with required legging-gap disclosure), and TCA measurements (slippage vs decision-mid/arrival-side, spread capture/concession, roll execution cost kept strictly additive to the old leg's immutable realized P&L per ROLL-001). Zero-bid/invalid-quote/wide-market states all handled honestly, never defaulted. See docs/research/THETA_EXECUTION_TCA_RESEARCH.md. 21 tests."),
    PhaseItem("R7_STRATEGY_SWITCH", PhaseState.COMPLETE, ("strategy_switch_research.py",),
              note="R7 P2C pass-2 directive: formalizes H-Q-04 into evaluate_strategy_switch() (STAY/SWITCH/WAIT). Structurally cannot accept a realized future outcome as its edge input, preventing 'destination profitable = switch was correct' by construction. Charges every SWITCH candidate for closing/entry execution cost and theta forgone; capital-days-forgone kept as a separate capacity dimension, never folded into the dollar cost. 7 tests."),
    PhaseItem("R7_SURFACE_QUALITY_GATE", PhaseState.COMPLETE, ("volatility_surface_research.py",),
              note="R7 P2C pass-2 directive: adds classify_surface_quality_state() to the existing raw-SVI module -- maps a SurfaceFitResult onto GOOD/THIN/ARBITRAGE_INCONSISTENT/UNKNOWN with an explicit reaction posture, keeping 'no fit' and 'a marginal fit' structurally distinct and blocking feature use on any arbitrage-inconsistent fit even when parameters are technically present. A bad surface never silently reads as zero skew. 5 new tests."),
    PhaseItem("R7_LABEL_QUALITY", PhaseState.COMPLETE, ("label_quality_research.py",),
              note="P2D directive section 11: a four-tier label-quality score (HIGH/MODERATE/LOW/UNUSABLE) for P2C's resolved outcome labels, reading only fields resolved-outcome-engine.ts already emits (provenance rank, completeness, exact-contract-identity presence, path/horizon completeness, event/corporate-action ambiguity flags). Hard ambiguity flags (unresolved corporate action, unreconciled contract adjustment) force UNUSABLE regardless of every other dimension; soft flags and structural gaps each degrade one tier. training_weight_for_tier() maps a tier to a caller-justified weight, never a built-in default scheme, and never returns a weight for UNUSABLE. See docs/research/THETA_P2D_EMPIRICAL_POLICY_LEARNING_DESIGN.md. 17 tests."),
    PhaseItem("R7_POSITION_PATH_STATE", PhaseState.COMPLETE, ("position_path_state_research.py",),
              note="P2D-continuation directive sections 7-10: canonical open-position path-state inventory (ALREADY_AVAILABLE/MISSING/DERIVABLE/PROVIDER_BLOCKED per field), compute_path_state() (peak/trough-by-PnL, giveback, time-since-entry/peak/trough from an ordered checkpoint list, no stateful mark step -- deliberately avoiding a real bug charlieyanhx/exitkit's own test suite found in an equivalent design), compute_trajectory() (one generic velocity/acceleration function reused across PnL/Delta/Gamma/IV/distance-to-strike), and assess_winner_to_loser() (the directive's own worked example -- a stable -5% position vs. one that peaked at +25% then fell -- verified to classify differently via a real test). Trade-path states are descriptive only, never wired to an automatic close. See docs/research/THETA_ACTION_INACTION_TIME_PATH_RESEARCH.md. 23 tests."),
    PhaseItem("R7_DECISION_FRESHNESS", PhaseState.COMPLETE, ("decision_freshness_research.py",),
              note="P2D-continuation directive sections 13-14: assess_decision_freshness() checks all ten directive-named invalidators (quote age, spot/IV move, Delta/Flow/GEX regime change, session/event-state transition, portfolio change, time decay) and reports every reason that fired, not just the first. Every numeric bound defaults to None (opt-in only) -- never a hyperactive-polling default; a caller supplying no bounds gets a decision that stays fresh regardless of magnitude, verified by a real test with a 999-point spot move. See docs/research/THETA_ACTION_INACTION_TIME_PATH_RESEARCH.md. 14 tests."),
    PhaseItem("R7_EXPLAINABILITY_SCHEMA", PhaseState.COMPLETE, ("explainability_schema_research.py",),
              note="P2E directive section 20: operator-facing explainability schema. render_explanation_text() is a pure, deterministic, template-based renderer (never an LLM call) answering what THETA chose, what else was feasible, why it won, what data was missing (REQUIRED_UNKNOWN vs OPTIONAL_UNKNOWN, reusing the existing severity taxonomy), what time/position-path/provider-fact context applied, and the risk of acting vs. not acting -- every field assembled from evidence this branch's or Codex's existing modules already produce, no new computation. See docs/research/THETA_P2E_OPERATOR_PROVIDER_QUALIFICATION_RESEARCH.md. 6 tests."),
