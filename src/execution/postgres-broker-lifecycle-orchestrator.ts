...
export async function applyConfirmedTerminalLifecycle(pool:Pool,connectionId:string,
  ...
  const rows=await pool.query(`SELECT ec.chain_id,ec.bot_instance_id,ec.lifecycle_state,ol.option_leg_id,ol.option_contract_id,
      ol.quantity AS original_contracts,(ol.quantity-COALESCE(pc.closed_quantity,0)) AS contracts,
      ol.entry_credit_debit,COALESCE(pc.realized_pnl,0) AS partial_realized_pnl,
      oc.contract_symbol,oc.option_type,oc.strike,oc.multiplier,u.symbol AS underlying,
    ...
    LEFT JOIN LATERAL(SELECT sum(closed_quantity) AS closed_quantity,
      sum(realized_pnl_before_fees) AS realized_pnl
  ...
  for (const row of rows.rows as Row[]) {
    const contracts=n(row.contracts),originalContracts=n(row.original_contracts),multiplier=n(row.multiplier),previousOption=n(row.previous_option_qty)??0,
      ...
      credit=n(row.entry_credit_debit),partialRealized=n(row.partial_realized_pnl),activityHash=s(row.provider_activity_ref_hash);
    if (contracts===null||originalContracts===null||multiplier===null||strike===null||credit===null||partialRealized===null
      ||activityHash===null||previousOption>=0||currentOption!==0) { unresolved++; continue; }
    const expectedShares=contracts*multiplier, occurredAt=s(row.activity_at)??observedAt;
    const terminalOptionPnl=partialRealized+credit*(contracts/originalContracts);
    ...
    if (activityType==='OPEXP') {
      ...
      application={...common,eventKind:'OPTION_EXPIRATION',optionLegId:String(row.option_leg_id),itm:false,realizedOptionPnl:terminalOptionPnl,
