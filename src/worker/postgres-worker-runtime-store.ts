...
export class PostgresWorkerRuntimeStore implements WorkerRuntimeStore {
  ...
  async register(input: WorkerRegistration): Promise<string|null> {
    ...
    [input.workerId,masterPaperLeaseKey,input.hostId,input.buildSha,JSON.stringify(input.strategyVersions),input.startedAt]);
    if(previous.rows[0]?.worker_id!==input.workerId)
      await this.event(input.workerId,'STARTED',input.startedAt,{buildSha:input.buildSha,runtimeMode:'MASTER_THETA_PAPER'});
  ...
  async acquireLease(workerId:string,at:string,expiresAt:string):Promise<'ACQUIRED'|'TAKEN_OVER'|'HELD_BY_OTHER'>{
    const previous=await this.pool.query(`SELECT worker_id,expires_at <= $2::timestamptz AS expired
      FROM ops.runtime_worker_lease WHERE lease_key=$1`,[masterPaperLeaseKey,at]);
    const result=await this.pool.query(`INSERT INTO ops.runtime_worker_lease(lease_key,worker_id,acquired_at,heartbeat_at,expires_at)
      ...
      RETURNING worker_id`,[masterPaperLeaseKey,workerId,at,expiresAt]);
  ...
  async heartbeat(workerId:string,at:string,expiresAt:string,state:WorkerRuntimeState):Promise<boolean>{
    ...
    [masterPaperLeaseKey,workerId,at,expiresAt]);
    ...
    await this.pool.query(`UPDATE ops.runtime_worker_status SET last_heartbeat=$2,state=$3,updated_at=$2,
      failure_reason=CASE WHEN $3 IN ('ERROR','MASTER_PAPER_RISK_BLOCKED','MASTER_PAPER_PROVIDER_DEGRADED',
        'MASTER_PAPER_PAUSED_BY_KILL_SWITCH') THEN failure_reason ELSE NULL END WHERE worker_id=$1`,
    [workerId,at,state]);
    return true;
  }

  async cycleStarted(workerId:string,at:string):Promise<void>{
    await this.pool.query(`UPDATE ops.runtime_worker_status SET state='MASTER_PAPER_RECONCILING',last_cycle_started=$2,
  ...
  async stop(workerId:string,at:string,state:'STOPPING'|'OFFLINE'|'ERROR',reason?:string):Promise<void>{
    ...
    if(state==='OFFLINE'||state==='ERROR'){
      await this.pool.query(`DELETE FROM ops.runtime_worker_lease WHERE lease_key=$1 AND worker_id=$2`,[masterPaperLeaseKey,workerId]);
    }
