...
export type DatabaseReadiness = {
  ...
  readonly required_migration: "007_connection_readiness";
...
export async function checkDatabaseReadiness(
  ...
  const base = {
    ...
    required_migration: "007_connection_readiness" as const,
  ...
  try {
    ...
    const result = await pool.query(`
      SELECT
        (SELECT version FROM core.schema_migration ORDER BY applied_at DESC LIMIT 1) AS latest_migration,
    ...
    return {
      ...base,
      state:
        latest === base.required_migration && objectRow.customer_iam === true && objectRow.token_vault === true
          ? "CONNECTED"
          : "MIGRATION_REQUIRED",
