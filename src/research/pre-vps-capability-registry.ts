...
export const capabilityRegistry: readonly CapabilityRecord[] = [
  ...
  },
  {
    ...
    maturity: 'PRODUCTION_REQUIRED', firstPaperRequired: true, preVpsRequired: true,
    producer: 'stressGapDetected is real; aegis-iv-stress.ts now produces stressIvShockDetected from exact-session Optionomics ATM IV with explicit baseline maturity; spread widening remains producerless',
    dataSource: 'Optionomics METRICS atm_iv for IV stress; no source yet wired for stressSpreadWideningDetected',
    persistence: 'market.optionomics_iv_session_observation + risk.aegis_iv_stress_assessment', consumer: '_system() (bots/theta/quant/models/aegis.py:155-166, HOLD_ONLY whenever ANY of 3 signals is None) AND _liquidity() (:129-135, HOLD_ONLY whenever stressSpreadWideningDetected is None)',
    ...
    blocker: 'P0 -- the IV producer is source-wired and authenticated Optionomics METRICS dates/values were verified, but stressSpreadWideningDetected remains null. LIQUIDITY and SYSTEM therefore still hold new risk. Exit supremacy (CLOSE/CANCEL/RECONCILE) is unaffected.',
    owner: 'CODEX',
    sourceFiles: ['bots/theta/quant/models/aegis.py:129-135,155-193,203-209', 'src/theta/aegis-iv-stress.ts', 'src/research/production-shadow-runtime.ts', 'src/theta/aegis-derivation.ts', 'src/theta/account-exposure.ts'],
  },
  {
