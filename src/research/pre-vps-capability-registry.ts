...
export const capabilityRegistry: readonly CapabilityRecord[] = [
  ...
  },
  {
    ...
    maturity: 'PRODUCTION_REQUIRED', firstPaperRequired: true, preVpsRequired: true,
    producer: 'canonical-strategy-frontier.ts (entry-time, real fallback derivation); MISSING for management-time context',
    ...
    runtimeReachable: 'PARTIAL', authority: 'src/theta/canonical-strategy-frontier.ts / src/theta/management-input-state.ts',
    mechanicallyTested: true, providerTested: false, noSubmitTested: false, empiricallyValidated: false,
    currentState: 'PARTIAL',
    blocker: 'ManagementInputState.context.assignmentCapacity has no real producer (riskState hardcoded null in theta-shadow-cycle.ts) -- structurally blocks ACCEPT_ASSIGNMENT on every real cycle. See THETA_ASSIGNMENT_CAPACITY_RESOLUTION.md for the exact Codex patch.',
    owner: 'SHARED', sourceFiles: ['src/theta/canonical-strategy-frontier.ts', 'src/theta/management-input-state.ts', 'src/theta/theta-shadow-cycle.ts', 'src/theta/management-action-frontier.ts'],
  },
  {
