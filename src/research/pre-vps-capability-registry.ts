...
export const capabilityRegistry: readonly CapabilityRecord[] = [
  ...
  },
  {
    ...
    maturity: 'PRODUCTION_REQUIRED', firstPaperRequired: true, preVpsRequired: true,
    producer: 'None for 2 of 3 signals -- confirmed via a direct line-by-line read of bots/theta/quant/models/aegis.py this pass (not just a grep): only stressGapDetected is real',
    dataSource: 'None wired for stressIvShockDetected/stressSpreadWideningDetected',
    persistence: 'N/A', consumer: '_system() (bots/theta/quant/models/aegis.py:155-166, HOLD_ONLY whenever ANY of 3 signals is None) AND _liquidity() (:129-135, HOLD_ONLY whenever stressSpreadWideningDetected is None) -- TWO separate families, both permanently HOLD_ONLY',
    ...
    mechanicallyTested: false, providerTested: false, noSubmitTested: false, empiricallyValidated: false,
    currentState: 'STUB_DEFAULT',
    blocker: 'P0 (promoted this pass from P1) -- stressIvShockDetected/stressSpreadWideningDetected always null, correctly represented as UNKNOWN (never false-defaulted), but this UNCONDITIONALLY forces new_risk_state to HOLD_ONLY or worse on every real Production AEGIS evaluation today, permitting ZERO new-risk-opening actions regardless of every other family and independent of CONTRACT_NOT_EXECUTABLE. Exit supremacy (CLOSE/CANCEL/RECONCILE) is unaffected.',
    owner: 'CODEX',
    sourceFiles: ['bots/theta/quant/models/aegis.py:129-135,155-193,203-209', 'src/theta/aegis-derivation.ts:24-27', 'src/theta/account-exposure.ts'],
  },
  {
  ...
  },
  {
    ...
    blocker: 'UPDATED (Wave 9, after main 1ca3e27): still structurally unreachable via managementPolicyEvidenceProvider, but the picture changed -- see ROLL_CC_CANDIDATE_SOURCE for the precise two-path finding.', owner: 'CODEX',
  ...
  },
  {
    ...
    maturity: 'PRODUCTION_REQUIRED', firstPaperRequired: true, preVpsRequired: true,
    producer: 'PARTIAL, UPDATED (Wave 9, verified against main 1ca3e27): ProductionPaperManagementCandidateSource (autonomous-runtime.ts:429) is now real and wired -- discovers real candidates and feeds managementStore.assembleAndPersistOpenChains(), so the persisted management input states now carry real candidate-discovery evidence. BUT the SEPARATE managementPolicyEvidenceProvider (autonomous-runtime.ts:353-354, still createPaperBootstrapManagementPolicyProvider() with zero args, still defaulting to noCandidates) is what actually feeds buildRuntimeManagementFrontiers() at line 440-442 -- the function that produces the real ROLL/SELL_CC action frontier. That second wire was NOT part of this merge.',
    dataSource: 'Real for candidate discovery/persistence (ProductionPaperManagementCandidateSource); still none wired for the policy-evidence/frontier path',
    persistence: 'Real -- candidateDiscovery is now passed into assembleAndPersistOpenChains and persisted on the management input state',
    consumer: 'ROLL_CC_CANDIDATE_VALUATION (still blocked) -- NOT the same consumer as assembleAndPersistOpenChains (now real)', runtimeReachable: 'PARTIAL',
    authority: 'autonomous-runtime.ts:353-354 (frontier path, still stub) / autonomous-runtime.ts:429 (discovery/persistence path, now real) / paper-bootstrap-management-policy.ts:1162-1165 (factory, unchanged default)',
    mechanicallyTested: true, providerTested: false, noSubmitTested: false, empiricallyValidated: false,
    currentState: 'PARTIAL', blocker: 'The remaining, narrower gap: pass a real candidate source (built from the same ProductionPaperManagementCandidateSource discovery, or the discovery result itself) into createPaperBootstrapManagementPolicyProvider(...) at autonomous-runtime.ts:354, instead of calling it with zero arguments. This is now a precise, small wiring task, not an open-ended P0 -- most of the real work (discovery, persistence) already landed.', owner: 'CODEX',
    sourceFiles: ['src/theta/autonomous-runtime.ts:353-354', 'src/theta/autonomous-runtime.ts:429', 'src/theta/production-paper-management-candidate-source.ts', 'src/theta/paper-bootstrap-management-policy.ts:1162-1165'],
  },
  {
