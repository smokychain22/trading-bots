...
export const ownershipAegisDataLineage: readonly OwnershipAegisDataLineageEntry[] = [
  {
    ...
    expectedSource: 'Python ownership model referenced by ownership-contract.ts header comment (bots/theta/quant/models/ownership_v0.py) -- not present in this TS repo, cannot be inspected from here',
    ingestionPath: 'new-risk-orchestrator.ts: invokeAndValidate(bridge, "ownership", {snapshotId, underlyingSymbol, timestamp, policy: ownershipPolicy, inputs: ownershipInputs}) -> parseOwnershipEvaluationResponse',
    ...
    snapshotField: 'point-in-time-evidence.ts candidateEvidenceSchema.ownership -- an OPAQUE jsonObject; no typed per-field shape is persisted, confirmed by direct inspection',
    ownershipConsumer: 'new-risk-orchestrator.ts market context (ownershipAcceptable) and per-candidate lattice request field (ownershipAcceptability); decision-assembly.ts fails the whole underlying closed to OWNERSHIP_UNAVAILABLE when null',
  ...
  ...(['LiquidityQuality', 'StructuralQuality', 'RecoveryQuality', 'TailQuality', 'EventAdjustment'] as const).map((component) => ({
    ...
    expectedSource: 'Same Python ownership model as ownability -- component-level raw-input mapping is not documented anywhere in this TS repo (confirmed gap)',
    ingestionPath: 'Same as ownership.ownability -- components arrive already-scored inside the same OwnershipEvaluationResponse payload, never separately fetched',
    normalizedField: `componentScoreSchema entry with name === '${component}' (theta/ownership-contract.ts)`,
    snapshotField: 'Not separately persisted -- only the collapsed ownership jsonObject blob exists in candidateEvidenceSchema, confirmed no per-component column',
    ownershipConsumer: 'ownability computation only (component values are never individually read downstream of ownership-contract.ts in this TS repo)',
  ...
  })),
  {
    ...
    snapshotField: 'point-in-time-evidence.ts candidateEvidenceSchema.aegis -- an OPAQUE jsonObject, confirmed no typed per-family column persisted',
  ...
  },
  {
    ...
    normalizedField: 'familyAssessmentSchema entries (theta/aegis-contract.ts) -- each family always has a concrete riskStateSchema value, never null; there is no per-family UNKNOWN representation in the contract, confirmed gap: family-level completeness cannot be diagnosed from this schema alone, only whole-assessment-present-vs-absent',
    snapshotField: 'Not separately persisted -- only the collapsed aegis jsonObject blob',
  ...
  },
  {
    ...
    expectedSource: 'Documented in aegis-derivation.ts file header as NOT YET IMPLEMENTED -- no sector-classification source, no cross-underlying correlation bars, no real IV/spread-history stress baseline exists',
    ingestionPath: 'Caller-supplied placeholders (e.g. false) passed into aegisInputs -- confirmed these are structurally-unimplemented placeholders, not genuine zero-evidence UNKNOWN signals, and are NOT currently flagged as UNKNOWN anywhere',
    normalizedField: 'N/A -- these never reach a typed normalized field distinct from the placeholder value',
    snapshotField: 'Embedded inside the opaque aegis inputs blob only',
    ownershipConsumer: 'None',
    aegisDependency: 'Consumed directly by the Python AEGIS model as if they were real evidence -- this is a confirmed evidence-integrity gap worth flagging to Codex: a placeholder false is indistinguishable from a verified false at every downstream consumer',
  },
];
