  ...
  switch (action) {
    ...
    case 'ALLOW_CALL_AWAY': {
      ...
      const referenceCallAwayPnl = proceeds === null || referenceBasis === null ? null : computeWholeChainPnl({
        ...
        stockSharesAssigned: state.economics.openStockShares, dividends: state.economics.dividends,
        fees: state.economics.fees,cashflowBasis:'BENCHMARK_CASHFLOW',executionCostNotEmbeddedInCashflows:null,
        tcaExecutionShortfall:null,coveredCallPremium: state.economics.entryCreditDebit,
        coveredCallCloseCosts: 0, stockSaleOrCallAwayProceeds: proceeds, currentStockMarkPerShare: null, openStockShares: 0,
      }).wholeChainPnl;
      return {
...
export class PaperBootstrapManagementPolicyProvider implements ManagementPolicyEvidenceProvider {
  readonly authority = 'PAPER_BOOTSTRAP_MANAGEMENT_POLICY' as const;
...
export function createPaperBootstrapManagementPolicyProvider(
  candidates: PaperBootstrapCandidateSource = noCandidates,
): ManagementPolicyEvidenceProvider {
  return new PaperBootstrapManagementPolicyProvider(candidates);
}
