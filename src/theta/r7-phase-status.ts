...
export function buildR7PhaseStatus(dryRun:FirstPaperOrderDryRun):R7PhaseStatus {
  ...
  const empiricalBlocked=dryRun.receipt.empiricalBlockers.some(isEmpiricalPolicyBlocker);
  ...
  return {receiptVersion:'theta-r7-phase-status-v1',R7_ENGINEERING:engineering,
    ...
    blockers:[...new Set([...engineeringBlockers,...dryRun.receipt.operationalBlockers,...dryRun.receipt.empiricalBlockers])].sort()};
}
