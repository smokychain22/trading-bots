...
export interface ThetaAlert {readonly identity:string;readonly type:ThetaAlertType;readonly severity:ThetaAlertSeverity;
  ...
  readonly lifecycleTransition?:'OPENED'|'REPEATED'|'RESOLVED'|'REOPENED';}
...
export function mergeAlert(previous:ThetaAlert|null,input:Omit<ThetaAlert,'firstSeenAt'|'occurrenceCount'>):ThetaAlert{
  if(previous!==null&&previous.identity===input.identity&&previous.state==='ACTIVE'&&input.state==='ACTIVE')
    return {...input,firstSeenAt:previous.firstSeenAt,occurrenceCount:previous.occurrenceCount+1,lifecycleTransition:'REPEATED'};
  const lifecycleTransition=previous===null?'OPENED':previous.state==='RESOLVED'&&input.state==='ACTIVE'?'REOPENED':
    previous.state==='ACTIVE'&&input.state==='RESOLVED'?'RESOLVED':'OPENED';
  return {...input,firstSeenAt:input.lastSeenAt,occurrenceCount:1,lifecycleTransition};
}
export async function persistAlert(pool:Pool,alert:ThetaAlert):Promise<void>{
  ...
  await pool.query(`INSERT INTO ops.theta_alert_event(alert_event_id,alert_identity,event_type,severity,source,first_seen_at,
    ...
    alert.lastSeenAt,alert.occurrenceCount,alert.state,alert.relatedRef,
    JSON.stringify({...alert.evidence,lifecycleTransition:alert.lifecycleTransition??null}),hash]);
}
