...
| Management assignment capacity | The missing `riskState.assignmentCapacity` dependency is removed from the canonical management input. The same secured-lot calculation used for entry now derives whole-contract capacity from a fresh persisted Alpaca account snapshot and the open put's actual strike/multiplier. Positive, zero, stale, missing and non-applicable states have focused fixture proof. This closes the source-level missing-producer defect; the old worker has not observed it. | Verify persisted account snapshot lineage and the ACCEPT_ASSIGNMENT frontier on a locked current-release worker after database recovery. Available buying power is a conservative incremental-capacity signal; broker-confirmed assignment reconciliation remains authoritative regardless of this prospective frontier. |
