...
The Production authenticated probe returned HTTP 200 for `company_catalyst`, with `events=[]`, a served window of 2026-09-21 through 2026-10-21, and complete pagination for that particular response. Its information state is `EMPTY_EVENTS_UNQUALIFIED`, not verified absence. The MCP qualification also returned HTTP 200 for `events` and `earnings_analyses`; the latter exposes historical filing analysis rather than a complete future earnings calendar. There is no qualified supported REST/MCP future company search from this probe. This is a specific provider capability gap for the current entry policy, not an authentication gap.
...
The first protected Production capture persisted five positive records for 20 governed candidate symbols across one complete response page. None was classified as a pending unsupported action. `negativeCoverageQualified` remained `false`. This is evidence that the positive producer and Aiven writer run, but the five non-blocking positives must not be turned into a negative assurance for the other symbols. Migration `064_alpaca_corporate_action_observation` was applied on Aiven, and post-migration validation reported 64 migrations, 158 tables, no invalid indexes, and no unvalidated constraints.
...
The first protected Production export contains six real market-wide event rows, zero ticker-scoped rows, and content hash `f8730b71dca521b85f60d4580c9dfe6b9872ac6683b2eeaff9219ecbef7a6d74`. Its `canonicalSourceSha` is `a0e69e1184f975c43a46c18bf55fe7d5cd8a6ca9`, verified against the deployed Vercel build and then-current canonical main. The local export capture writes the sanitized artifact to ignored `research_exports/canonical-events/`, verifies the content hash and evidence-ID uniqueness, and refuses an export whose release SHA differs from the fetched `origin/main`. The artifact is not a company-earnings dataset and must not be used to claim that the four entry-critical UNKNOWNs are closed.

The full local verification for `a0e69e1` passed: Node 1,410 pass and 11 skip, Python 564 pass plus five subtests, TypeScript check, lint, build, security scan, and 23 browser tests. GitHub CI run `35615406284` passed. Production deployed the commit and remained operational with the prior worker release and new-risk lock. Aiven's protected validation reported zero local order intents, broker-order rows, and fills. These database counts are not a fresh Alpaca broker order-history query.
