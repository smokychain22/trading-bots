...
| Management snapshot + action frontier | `trade.management_input_snapshot` LEFT JOIN `trade.management_action_frontier`, plus immutable `research.theta_shadow_management_policy_evidence` | `ManagementSnapshot` with actions and `shadow_policy_evidence` embedded | mixed | `observed_at` | `actions=()` if the frontier was never computed, `{}` if no P2 shadow record exists | MATCH | Loader maps the current TypeScript frontier keys and rejects a shadow record unless execution and comparison remain locked |
...
| Options-chain decision evidence | `research.theta_option_chain_decision_evidence` and `rows.optionChainDecisions[]` | validated immutable dictionary rows in `production_export_loader.py` | mixed | `observedAt` is the feature cutoff | outcomes and label timestamps remain null | MATCH | Research loader rejects execution authority, empirical readiness, or a populated future label |
| Dataset export identity | `datasetExportVersion = "theta-r6-dataset-v3"` (TS constant) | `DATASET_SCHEMA_VERSION` (Python constant, `production_export_loader.py`) | string | n/a | n/a | MATCH | Loader rejects any other value outright |
| Dataset hash | `sha256(canonicalJson(unsigned))` (TS) | `sha256_hex(canonical_json(unsigned))` (Python, same algorithm) | hex string | n/a | never null | MATCH | Python reproduces TypeScript's Unicode, ECMAScript number-formatting, and v1 `localeCompare` key-ordering semantics. The real Production export hash `25478c74...f32da` was independently recomputed byte-for-byte on 2026-09-14. |
