...
**2026-09-18 update, ported onto `claude/theta-management-challenger` (post-C1-integration):**
Codex has since landed `src/theta/python-bridge.ts` (a controlled, allowlisted,
fail-closed Python↔TS bridge explicitly scoped as "research/shadow" only, never the
live Production management-policy authority) and `src/theta/whole-chain-component-
evidence.ts` (explicitly documented as "Structurally compatible with Claude's
WholeChainComponents contract"). This directly addresses §1's finding below: the
TypeScript management-policy path remains the one live decision authority, and the
Python quant package is now reachable only through a controlled, non-authoritative
bridge. §1 is kept below as the historical record of the finding and is no longer an
open architecture question requiring resolution — it was resolved by Codex's own
subsequent work, not by this document.
