...
**Status: RESEARCH_ONLY.** GEX is not wired into THETA's feature set
(`FeatureFamily.GEX` = `NOT_IMPLEMENTED` in `bots/theta/quant/research/ablation.py`).
...
this phase — GEX has no THETA feature-family implementation yet
(`FeatureFamily.GEX = NOT_IMPLEMENTED`), no real historical options-chain data exists
...
above and name it precisely in code/docs (never "gamma flip" unqualified), (2) run it
through `ablation.py`'s `classify_ablation_result` against a baseline without it
before any promotion, (3) never let two dashboard panels compute "zero gamma" by two
