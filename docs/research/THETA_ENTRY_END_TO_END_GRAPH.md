...
| executable BBO / multiplier gate | `option-contract.ts:210-224` (real 10-condition gate), `option-chain-ingestion.ts:147-160` | REAL, root cause RESOLVED Wave 4 | WIRED | -- | REACHABLE | Unclear | 3,299/3,876 candidates rejected `CONTRACT_NOT_EXECUTABLE` in the one real session -- Codex's own real-data query (`docs/operations/THETA_PRODUCTION_CLOSURE_WAVE1_2026-09-22.md`) confirms the exact cause: 2,014 quote-stale+spread-wide, 787 spread-wide, 498 quote-stale. This is the gate working correctly, not a defect; whether its calibration (30s age, max spread) is right for Paper remains a separate open Codex/owner policy question -- see `THETA_CONTRACT_NOT_EXECUTABLE_INVESTIGATION.md` |
