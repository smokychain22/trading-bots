...
The isolated Production-environment deployment at source `ab58650d167dd0e40926589d0ba41aba343a894b` authenticated both REST and MCP against the configured Vega account. It ran 18 read-only REST probes, all returning HTTP 200, and discovered 23 MCP tools. The immutable sanitized qualification receipt was written to Aiven after migration `063_optionomics_event_first_observation`. This proves account reachability and sampled response shapes. It does not prove continuous coverage, field units, provider observation timestamps, historical depth, or trading edge.

The SPY options and metrics historical requests for `2026-09-18` each returned that exact served session and populated data. Option analytics such as bid, ask, strike, implied volatility, Greeks and `iv_per_trading_day` arrived as strings. The deprecated `iv_per_day`, `vrp_20` and `earnings_in_sessions` were null in the sampled response. SPY price history returned a candle series without a top-level served session. A request-date comparison against a missing top-level date would misclassify this series, so the qualifier now checks whether the requested trading session occurs among returned candles. Heatmaps returned a metric field, which the qualifier now compares with the requested gamma, Vanna or Charm metric. Current and bounded historical event requests returned empty event arrays with pagination metadata. This is not evidence that the queried event families are absent or that prospective earnings coverage is complete. The earnings-filings response contained an empty filings array in the sampled account response, with no proof of upcoming earnings coverage. Flow aggregates and net series were populated but lacked a top-level session timestamp in the sampled response. News articles carried `published_at` and sentiment `analyzed_at`; disclosure trades carried `filed_at`, transaction date and nullable lifecycle timestamps. These are distinct clocks.

The Optionomics qualification receipt holds schema and information-state metadata, not the provider payload, credentials or a real-data research export. No Claude event, correlation, IV, executable-BBO or downside contract has been filled from this probe. No research result has been promoted to Paper authority. Optionomics session bid/ask remains analytical evidence; Alpaca is still the broker and current executable BBO authority. The canonical worker was not restarted for this qualification.

The final isolated deployment reran the authenticated 18-operation probe after the semantic checks changed. Historical options and metrics both matched the requested `2026-09-18` session. The requested session appeared in price-history candles. Gamma, Vanna and Charm heatmaps each echoed the requested metric. The historical events route echoed its bounded date window and reported pagination complete, but its sampled event array was empty. This remains `EMPTY_EVENTS_UNQUALIFIED` because family and prospective earnings completeness were not established. The final sanitized receipt hash was `3914b5f5bab67ca3077aa498f1448d83b4f76758705e8c5f9a15cc19c51c4f7e`.
...
The authenticated Production rerun at 2026-09-14T12:19Z persisted 37 capability results. All 37 returned or inherited an HTTP 200 `GOOD` transport state. The three exposure requests explicitly used gamma, Vanna, and Charm metrics and returned populated shapes with 693, 669, and 669 sampled array elements respectively. Rate-limit limit, remaining and reset headers were present. Transport health doesn't prove quantitative usefulness, sign convention, units, or execution fitness. The earlier current SPY chain was populated and contained 12,838 sampled array elements. This is a bounded schema-census count, not a promise that every element is a distinct executable quote. The dated historical chain for 2026-09-07 returned 11,966 direct option observations. Current and dated metrics shared the same schema fingerprint. Dated metrics returned 83 named metric values. The 8h, 24h and 48h historical net-flow windows returned zero points. Historical event context returned two events with two `known_at` timestamps.
...
|---|---|---|
| `OPTIONOMICS_ENDPOINT_CENSUS` | COMPLETE_WITH_LIMITS | 37 authenticated results persisted. ID-child routes and POST assessment remain deliberately uncalled. |
...
| `OPTIONOMICS_VANNA` | COMPLETE_WITH_LIMITS | Authenticated Production GET returned HTTP 200 with a populated `vanna_exposure` heatmap shape. The typed adapter, exact response-metric guard, lineage record, feature destination, and tests are complete. Provider methodology, units, sign convention, and session-time semantics remain unverified, so the grid remains research context. |
| `OPTIONOMICS_CHARM` | COMPLETE_WITH_LIMITS | Authenticated Production GET returned HTTP 200 with a populated `charm_exposure` heatmap shape. The typed adapter, exact response-metric guard, lineage record, feature destination, and tests are complete. Provider methodology, units, sign convention, and session-time semantics remain unverified, so the grid remains research context. |
