...
| `CONTRACT_MULTIPLIER_MAPPING` | HARD_REQUIRED_SAFETY | true | NOT_INDEPENDENTLY_VERIFIED | A wrong multiplier silently misprices collateral/premium by the multiplier's ratio. The persisted September 21 rejection details instead identify quote staleness and/or spread width in all 3,299 `CONTRACT_NOT_EXECUTABLE` rows, so multiplier remains a separate live-metadata check, not the proven cause of that cohort. |
...
| `AEGIS_SYSTEM_LIQUIDITY_STRESS` | HARD_REQUIRED_SAFETY | true | STUB_DEFAULT | Missing IV-shock or spread-widening evidence forces SYSTEM HOLD_ONLY; missing spread evidence also forces LIQUIDITY HOLD_ONLY. The first-Paper applicability policy remains unresolved. |
