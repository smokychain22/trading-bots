...
## 2026-09-18: Keep one decision authority and add a sanitized local recovery ledger

- Decision authority: The canonical strategy frontier and common management frontier remain the only economic selection paths. Plan assembly, execution handoff, broker coordination, lifecycle, and accounting may validate or translate a decision but may not re-rank it.
- Management provider: One `ManagementPolicyEvidenceProvider` is active in a cycle. The bounded Paper bootstrap provider remains active until an independently reviewed and explicitly promoted empirical provider replaces it. Bootstrap and promoted providers cannot compete in one Production cycle.
- Orchestration: XState, Temporal, Bitloops, OpenTelemetry, Testcontainers, fast-check, BullMQ, and Cockatiel were reviewed at pinned source commits. No new runtime framework closes a current first-canary blocker. PostgreSQL leases, checkpoints, immutable action plans, deterministic IDs, and reconcile-before-retry remain canonical.
- Local durability: Aiven remains the only transactional runtime database. The Windows supervisor writes a sanitized append-only, SHA-256 hash-chained operational receipt after each bounded cycle. The allowlist excludes credentials, customer or broker identifiers, symbols, positions, and raw provider payloads.
