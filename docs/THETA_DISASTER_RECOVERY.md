...
Production migrations on the local canonical Aiven connection must use `tools/windows/dr/Invoke-ThetaProductionMigration.ps1`. It requires a verified, test-restored backup before migration, runs schema/invariant verification afterward, then takes and test-restores a post-migration backup. The ordinary `tools/database-migrate.mjs` refuses Aiven unless called inside this checkpoint workflow. The legacy remote bootstrap migration operation is disabled once Aiven is the production runtime authority. If a migration or post-check fails, the pre-migration backup remains preserved and no automatic destructive rollback is attempted. Do not run production migrations while Aiven is read-only.
...
Three full production backups are verified. The Task Scheduler-produced archive `C:\ProjectBackups\trading-bots\daily\2026-09-21_161458-0d9ca7c0` is the current `latest`, with custom archive SHA-256 `d0d1ad125778d1eeccbba7a774b96fb24b0cf56c25e98aad761b426921be3973`. Its source Git SHA is `3356b47d3dd9c510036e114186d1b0beb7c21d67`. A fresh PostgreSQL 18 local database restored 12 schemas, 153 base/partitioned tables, five views, 46 functions, 112 user triggers, 395 indexes, 17 sequences, two extensions, and migration `064_alpaca_corporate_action_observation`. It also restored two customer identities, two encrypted broker credentials, 25,126 legacy artifacts, and verified the copied external-asset files byte-for-byte. The different earlier trigger/sequence counts came from `information_schema` versus `pg_catalog` counting conventions. Both source and target in the restore test use the same `pg_catalog` definitions.
...
The task `THETA-Aiven-Verified-Backup` was registered on 2026-09-21 for 02:15 Asia/Karachi. A manual Task Scheduler launch at 21:14 completed at 21:22 with `LastTaskResult=0`, published `latest`, and passed an independent fresh local restore. The next time-based scheduled trigger, reported for 2026-09-22 02:15, has not yet occurred and must still be observed.

The first scheduler test was queued because Windows' default task settings disallowed battery power. After allowing battery use, WSL-backed `pg_dump` was interrupted during a manually started task. Neither attempt replaced `latest`; their incomplete staging folders were moved to `restore-tests` for diagnosis. The unattended production backup now uses native PostgreSQL 18.6 Windows client binaries from the [PostgreSQL Windows binary distribution](https://www.postgresql.org/download/windows/). The subsequent task-launched run, verification, and isolated restore passed. Its successful manual start proves the task action and permissions, while the actual 02:15 timer remains unobserved.
