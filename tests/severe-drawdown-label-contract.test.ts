...
test('CORE CLAIM: a resolved chain with no breach is FACTUAL_NEGATIVE, never censored', () => {
  const result = buildSevereDrawdownLabel({ chainId: 'c1', threshold: THRESHOLD, peakAdverseExcursionFraction: 0.2, isResolved: true });
  assert.equal(result.label, 'FACTUAL_NEGATIVE');
});

test('a resolved chain with a breach is FACTUAL_POSITIVE', () => {
  const result = buildSevereDrawdownLabel({ chainId: 'c2', threshold: THRESHOLD, peakAdverseExcursionFraction: 0.7, isResolved: true });
  assert.equal(result.label, 'FACTUAL_POSITIVE');
});

test('an open chain is RIGHT_CENSORED regardless of its interim path', () => {
  const result = buildSevereDrawdownLabel({ chainId: 'c3', threshold: THRESHOLD, peakAdverseExcursionFraction: 0.9, isResolved: false });
...
test('a resolved chain with no path evidence at all is censored, not fabricated negative', () => {
  const result = buildSevereDrawdownLabel({ chainId: 'c4', threshold: THRESHOLD, peakAdverseExcursionFraction: null, isResolved: true });
  assert.equal(result.label, 'RIGHT_CENSORED');
});

test('the threshold is a versioned research parameter, never hardcoded inside this module', () => {
  const looser = buildSevereDrawdownLabel({ chainId: 'c5', threshold: { thresholdVersion: 'v2', severeDrawdownFraction: 0.9 }, peakAdverseExcursionFraction: 0.7, isResolved: true });
