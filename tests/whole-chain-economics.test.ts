...
const components = (overrides: Partial<WholeChainComponents> = {}): WholeChainComponents => ({
  cashflowBasis:'ACTUAL_FILL_CASHFLOW',initialPutPremium: 200,putCloseCosts:0,rollCredits: 50, rollCloseCosts: 30, assignmentStrike: 195, stockSharesAssigned: 100,
  dividends: 0, coveredCallPremium: null, coveredCallCloseCosts: null, stockSaleOrCallAwayProceeds: null,
  fees:2,executionCostNotEmbeddedInCashflows:1,tcaExecutionShortfall:7,
...
test('computeEffectiveStockBasis lowers the strike by net retained premium per share and actual non-embedded costs', () => {
...
test('computeEffectiveStockBasis names every missing component rather than guessing', () => {
  const result = computeEffectiveStockBasis(components({ rollCredits: null, executionCostNotEmbeddedInCashflows: null }));
  assert.equal(result.complete, false);
  assert.deepEqual(result.missingComponents, ['rollCredits', 'executionCostNotEmbeddedInCashflows']);
});
...
test('UNKNOWN != ZERO: unverified non-embedded execution cost makes canonical basis UNKNOWN', () => {
  const withUnknownSlippage = computeEffectiveStockBasis(components({ executionCostNotEmbeddedInCashflows: null }));
  assert.equal(withUnknownSlippage.complete, false);
  assert.deepEqual(withUnknownSlippage.missingComponents, ['executionCostNotEmbeddedInCashflows']);
  const withRealZeroSlippage = computeEffectiveStockBasis(components({ executionCostNotEmbeddedInCashflows: 0 }));
...
test('actual fill cash P&L never subtracts adverse or favorable TCA a second time',()=>{
  const adverse=computeWholeChainPnl(components({initialPutPremium:200,putCloseCosts:150,rollCredits:0,rollCloseCosts:0,
    assignmentStrike:null,stockSharesAssigned:0,dividends:0,coveredCallPremium:0,coveredCallCloseCosts:0,
    stockSaleOrCallAwayProceeds:null,fees:0,executionCostNotEmbeddedInCashflows:0,tcaExecutionShortfall:10,
    currentStockMarkPerShare:null,openStockShares:0}));
  assert.equal(adverse.wholeChainPnl,50);
  assert.equal(adverse.tcaExecutionShortfall,10);
  const favorable=computeWholeChainPnl(components({
    initialPutPremium:200,putCloseCosts:150,rollCredits:0,rollCloseCosts:0,assignmentStrike:null,stockSharesAssigned:0,
    dividends:0,coveredCallPremium:0,coveredCallCloseCosts:0,stockSaleOrCallAwayProceeds:null,fees:0,
    executionCostNotEmbeddedInCashflows:0,tcaExecutionShortfall:-5,currentStockMarkPerShare:null,openStockShares:0}));
  assert.equal(favorable.wholeChainPnl,50);
  assert.equal(favorable.tcaExecutionShortfall,-5);
});
...
test('CASH-FLOW IDENTITY: wholeChainPnl equals the exact sum of every actual leg cash flow, for a full CSP-to-call-away chain', () => {
  ...
  const chain = components({
    ...
    fees:6,executionCostNotEmbeddedInCashflows:3,tcaExecutionShortfall:10,
    currentStockMarkPerShare: null, openStockShares: 0,
  });
  ...
  const expected =
    ...
    - 6 - 3;       // fees + actual non-embedded execution cost
