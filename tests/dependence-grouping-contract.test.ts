...
test('CORE CLAIM: every leg of the same whole chain gets the same dependence-group id', () => {
  const assignments = assignDependenceGroups([
    { rowId: 'r1', wholeChainId: 'wc1', underlying: null, exposureWindowStart: null, exposureWindowEnd: null, marketEventClusterId: null },
    { rowId: 'r2', wholeChainId: 'wc1', underlying: null, exposureWindowStart: null, exposureWindowEnd: null, marketEventClusterId: null },
    { rowId: 'r3', wholeChainId: 'wc2', underlying: null, exposureWindowStart: null, exposureWindowEnd: null, marketEventClusterId: null },
  ]);
...
test('CORE CLAIM: repeated scans (same chain, many rows) do not inflate independentN', () => {
  const rows = Array.from({ length: 20 }, (_, i) => ({
    rowId: `scan${i}`, wholeChainId: 'wc-repeated', underlying: null, exposureWindowStart: null, exposureWindowEnd: null, marketEventClusterId: null,
  }));
