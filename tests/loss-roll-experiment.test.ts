...
const roll = (overrides: Partial<RollCashflowIdentity> = {}): RollCashflowIdentity => ({
  ...
  fees: 4,
  slippage: 3,
  wholeChainNetPnlImmediatelyAfterRoll: -107,
  ...overrides,
});
...
test('builds a research-only comparison conditioned on the canonical loss-cause assessment', () => {
  ...
  assert.equal(result.contractVersion, 'theta-loss-roll-experiment-v1');
...
test('ROLL preserves old loss and applies new cashflow without resetting history', () => {
  const result = buildLossRollComparisonRow(row({
    challenger: economics({ action: 'ROLL', roll: roll() }),
  }));
  assert.equal(result.challenger.roll?.oldLegRealizedPnl, -125);
  assert.equal(result.challenger.roll?.netCashflow, 18);
  assert.equal(result.challenger.roll?.wholeChainNetPnlImmediatelyAfterRoll, -107);
});
