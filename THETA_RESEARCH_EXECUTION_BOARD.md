...
| W18-1 | Universe discovery funnel diagnostic REALIGNED | DONE | `src/research/universe-discovery-funnel.ts` v2, 11 tests -- now imports Codex's real `UniverseDiscoveryStageDiagnostic` type directly (`src/theta/universe-discovery.ts`) instead of a speculative enum; cannot drift out of sync again (a Codex stage rename breaks the TS import, forcing an explicit fix). `firstEmptyStageIsProviderFailure` distinguishes a healthy VALID_EMPTY/READY zero from a real provider failure at the same stage -- explicitly does NOT convert either into a "no opportunity"/"system ready" claim (no such field exists on the result type). Correction-needed note removed. |
...
| W18-2 | Profit-taking experiment | DONE | `src/research/profit-taking-experiment.ts`, 10 tests -- all 6 policies (incl. TAKE_50) validated identically, no privileged policy; a NOT_IDENTIFIABLE counterfactual structurally cannot carry a real incremental value; closeDebit/remainingExecutablePremium kept distinct; no single-leg PnL field exists anywhere in the schema |
| W18-3 | Loss/roll experiment | DONE | `src/research/loss-roll-experiment.ts`, 12 tests -- HOLD/CLOSE/ROLL/LET_EXPIRE/ACCEPT_ASSIGNMENT use one common future horizon and the canonical eight-cause taxonomy; ROLL preserves old realized P&L separately and enforces `oldLegRealizedPnl + netCashflow = wholeChainNetPnlImmediatelyAfterRoll`, so a new credit cannot erase an old loss; research-only, broker authority false |
| W18-4 | Recovery/covered-call experiment | DONE | `src/research/recovery-covered-call-experiment.ts`, 10 tests -- RECOVERY_WAIT/SELL_STOCK/SELL_CC and HOLD_CC/CLOSE_CC/ROLL_CC/ALLOW_CALL_AWAY compare at one common horizon using canonical whole-chain economics; premium alone cannot select SELL_CC; retained/lost upside, call-away identity, CSP-adjusted basis, censoring, dividend/event missingness, fees, slippage, TCA, and capital days stay explicit; no selected action and broker authority false |
...
| W8-8 | Loss/recovery/CC downstream research | DONE | Loss/roll experiment is complete at W18-3 and recovery/covered-call experiment is complete at W18-4. Canonical real-data exports remain a separate wiring task. |
