...
| W15-12 | Universe discovery funnel diagnostic | DONE, STAGE-NAME CORRECTION NEEDED | `src/research/universe-discovery-funnel.ts`, 7 tests -- analyzer logic remains valid. Main `61f2108` ("Diagnose bounded universe stages and distinguish Alpaca auth failures") shipped the REAL Production stage diagnostic (`UniverseDiscoveryStageDiagnostic`, `src/theta/universe-discovery.ts`) with only 4 real stages (`SOURCE_ASSETS`/`EXCHANGE_FILTER`/`STOCK_BARS`/`OPTIONABILITY`), coarser than this module's speculative 11-stage list. A future pass should realign `universeDiscoveryStages` to the real 4 (or however many Codex's real funnel actually has) rather than the invented list -- the generic `analyzeUniverseFunnel`/`summarizeStageRates` logic itself needs no change, only the stage enum. Not corrected this pass (found at session end, noted for continuity). |
